...
Varpropsubstr <- sqldf(" SELECT Varprop FROM Argumentlistenbackup300621 WHERE Pos IN('Funktion_2') AND Nummer > 1 ")
Varpropsubstr <- Varpropsubstr$Varprop
Progvarsublist <- c(Varpropsubstr, Progvartext)
Substrtext <- data.frame(Progvarsublist)
print('Ergebnis in Variable Substrtext')
